        ...
        */
            ...
            if (! empty($filter_combo) ) {
                foreach ($filter_combo as $k => $v) {
                    ...
                    if (in_array($k, $true_false_array) ) {
                        ...
                        $other_arr = array( 'other_project', 'other_topic' );
        ...
        if (true === $continue_query ) {
            ...
            if ($q->have_posts() ) {
              ...
              <a href="#top" class="cta-btn back-to-top border cta-scroll main">Back to Top </a>
          </div>
                    <?php
                }
...
?>
    <div class="pccb-row">
        <div class="pccb-ro08w-label"><a href="<?php echo esc_url($the_url); ?>" data-part="text" data-num="<?php echo esc_attr($rand_num); ?>"<?php echo esc_attr($target); ?> class="card-link card-link-text-<?php echo esc_attr($rand_num); ?><?php echo esc_attr($xtra_class); ?>"<?php echo esc_attr($xtra_attr); ?>><?php echo esc_attr($link_text); ?></a></div>
        <div class="pccb-row-icon"><a href="<?php echo esc_url($the_url); ?>" data-part="icon" data-num="<?php echo esc_attr($rand_num); ?>"<?php echo esc_attr($target); ?>  class="card-link asset-link card-link-icon-<?php echo esc_attr($rand_num); ?><?php echo esc_attr($xtra_class); ?>"<?php echo esc_attr($xtra_attr); ?>><i class="icon-<?php echo esc_attr($i_class); ?>"></i></a></div>
    </div>
<?php
...
{
    $types      = array( 'case_study', 'video', 'report', 'one_pager', 'presentation', 'presentation_template', 'logobrand_guidelines', 'brochure', 'case_study_video', 'case_study_written', 'case_study_micro_business', 'case_study_small_business', 'case_study_women_entrepreneur', 'narrative_key_messaging', 'audio', 'research', 'asset_image' );
...
function Repo_Determine_Topic_data($ID)
{
    ...
    $allowed_priority_values = array( 'Economic Impact', 'Social Impact', 'Safety', 'Digital Citizenship', 'Policy', 'WhatsApp', 'Instagram' );
    ...
    if (is_array($meta)) {
        if ( array_key_exists('priority_topic', $meta) && !empty($meta['priority_topic']) ) { 
            if ( in_array($meta['priority_topic'][0], $allowed_priority_values) ) {
                ...
                if ('Social Impact' == $meta['priority_topic'][0]) {
                    $topic_class = 'topic-social-impact';
                    $topic_name  = 'Social Impact';
                }
                if ('Safety' == $meta['priority_topic'][0]) {
                    $topic_class = 'topic-safety';
                    $topic_name  = 'Safety';
                }
                if ('Digital Citizenship' == $meta['priority_topic'][0]) {
                    $topic_class = 'topic-digital-skills';
                    $topic_name  = 'Digital Citizenship';
                }
                if ('Policy' == $meta['priority_topic'][0]) {
                    $topic_class = 'topic-government-politics';
                    $topic_name  = 'Politics &amp; Government Outreach';
                }
                if ('Instagram' == $meta['priority_topic'][0]) {
                    $topic_class = 'topic-instagram-asset';
                    $topic_name  = 'Instagram';
                }
                if ('Whatsapp' == $meta['priority_topic'][0]) {
                    $topic_class = 'topic-whatsapp-asset';
                    $topic_name  = 'WhatsApp';
                }
    ...
    $topics      = array( 'economic_impact', 'social_impact', 'digital_citizenship_digital_skills', 'government_politics', 'safety' );
 ...
 * @return mixed
 */
function fbapac_create_hashed_media_url($post_id) {
 ...
 */
function fbapac_attachment_url_to_path( $url ) {
