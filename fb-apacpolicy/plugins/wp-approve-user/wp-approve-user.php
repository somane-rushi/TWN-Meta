	...
	public function wpau_approve( $user_id ) {
		...
		if ( ! get_user_meta( $user_id, 'wp-approve-user-mail-sent', true ) ) {
			...
			$sent = wp_mail(
				...
				sprintf( esc_html_x( 'Permission has been approved', 'Blogname', 'wp-approve-user' ), $blogname ),
				//$this->populate_message( $this->options['wpau-approve-email'], $user )
				'You have been approved at '.$blogname. ', please visit and click on the Facebook login again to get access to the site.<br/><br/>Thank you',
				$headers
			);
