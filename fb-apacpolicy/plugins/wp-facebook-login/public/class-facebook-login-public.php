...
class Facebook_Login_Public {
	...
	public function print_button() {
		$curr_url  = strtok( esc_url_raw( (is_ssl() ? 'https://' : 'http://') . (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '') . (isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '') ), '?' );
		$redirect  = ! empty( $_GET['redirect_to'] ) ? esc_url_raw( $_GET['redirect_to'] ) : $curr_url;
		// if we are in login page we don't want to redirect back to it
		if ( isset( $GLOBALS['pagenow'] ) && in_array( $GLOBALS['pagenow'], array( 'wp-login.php', 'wp-register.php' ), true ) && empty( $_GET['redirect_to'] ) ) {
			$redirect = '';
		}
	...
	public function print_disconnect_button( ) {
		...
		$redirect = apply_filters( 'flp/disconnect_redirect_url', esc_url_raw( ( is_ssl() ? 'https://' : 'http://' ) . (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '') . (isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '') ) );
	...
	public function disconnect_facebook( ) {
		...
		// refresh page
		wp_redirect( esc_url_raw( (isset($_GET['redirect'])) ? $_GET['redirect'] : '' ) );
		exit();
	}
