<?php
/**
 * Submodule: SITE SEARCH (Full Page)
 * This section appears as part of PAGEFIELDS
 **************************************************************************************************
 * This was made specfically for the /search/ page, and should probably only be used on that page.
 **************************************************************************************************
 *
 * @package fbsafety
 */
...
if ( isset($args['sub_fields']) && !empty($args['sub_fields']) ) :
	...
	$full_search_toggle = fbsafety_fm_get_data($sub_fields, 'toggle');
	if ('1' === $full_search_toggle) :
...
<div class="page-search-wrap">
  <div class="container">
    <div class="page-search-container">
      ...
      <form action="" id="full--search" method="post">
        <div class="search-tool">

          <input type="text" id="search--query-f" placeholder="<?php echo esc_attr($search_placeholder); ?>" autocomplete="off">
...
<script>
jQuery(document).on( 'submit', 'form#full--search', function(e) {
  e.preventDefault();
  fbsafety_search_results('', 'form#full--search', '#search--query-f', '#search--results-f', 'v3');
});
jQuery('form#full--search #search-submit').click( function(e) {
  e.preventDefault();
  fbsafety_search_results('', 'form#full--search', '#search--query-f', '#search--results-f', 'v3');
});
<?php
  if ( isset($_GET['ss']) ) :
    $sent_query = sanitize_text_field( $_GET['ss'] );
?>
jQuery(document).ready(function($){
  var query = decodeURIComponent( '<?php echo rawurlencode($sent_query); ?>' );
  fbsafety_search_results(query, 'form#full--search', '#search--query-f', '#search--results-f', 'v3');
});
<?php
  endif;
?>
...
?>
      <script>
        var the__list_f = JSON.parse( decodeURIComponent( '<?php echo rawurlencode( wp_json_encode($search_terms_explode ) ); ?>' ) );
        fbsafety_autocomplete(document.getElementById("search--query-f"), the__list_f);
      </script>
<?php
      endif;
    endif;
...
<div class="container small">
  <div class="row">
    ...
    <div class="search-results">
      ...
      <div class="search-results" id="search--results-f"></div>
