...
function vpn_ip_check() {
  ...
  if ( is_user_logged_in() || is_page('login-page') ) {
    ...
    if (false === $vpn_result) {
      if ( ! is_page('login-page') && !is_page('registration') && !is_page('login') && !is_page('external-request-form') && !is_page('facebook-employee-form') ) {
        // Hotfix see: https://wordpressvip.zendesk.com/tickets/115966
        wp_safe_redirect(
...
add_filter('fbl/notify_new_registration_mail','__return_false');
...
/**
 * LOOKBACK (ANNUAL REPORT) custom functions
 */
require get_template_directory() . '/lookback/functions_lookback.php';


/**
 * Thailand custom functions
 */
require get_template_directory() . '/thailand/functions_thailand.php';
...
function complete_fb_employee() {
  ...
  if ( 1 > count( $reg_errors->get_error_messages() ) ) {
    ...
    $headers = array('Content-Type: text/html; charset=UTF-8');
				wp_mail(
					$email,
					'Facebook APAC Policy Resource Hub - Access Request',
					'Your request for access to the Facebook APAC Policy Resource Hub has been received. Your request will be approved shortly.<br/><br/>Thank you,<br/><br/>APAC Policy Team',
					$headers
				);
